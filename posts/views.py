...
def ret_post(request, post_id):
	...
	if len(this_post) == 0:
		...
		if this_post[0].thread != 0:
			oppost = Post.objects.filter(id = this_post[0].thread)
			return HttpResponseRedirect('/post-'+str(oppost[0].id)+'#'+str(this_post[0].id))
		else:
			oppost = Post.objects.filter(id = this_post[0].id)
			this_post = 0
		posts_list = Post.objects.filter(thread = oppost[0].id)
		if request.method == 'POST': #work with forms.
			...
			post_form = PostForm()
	return render(request, 'posts/post.html', {'curr_board' : curr_board, 'board_list': board_list, 'topic' : oppost[0], 'post_form': post_form, 'posts_list': posts_list})

def ret_board_with_catalog(request, board_name):
	return ret_board(request, board_name)

def ret_board_page(request, board_name, page_num):
	return ret_board(request, board_name, page_num)
	
def ret_board(request, board_name, page_num = 1):
	board_list = Board.objects.filter(name = board_name)
	board_id = board_list[0].id
	all_post = Post.objects.filter(board = board_id)
	topics_list = []
	for thread in ThreadPost.objects.all():
		if thread.op.board.id == board_id:
			topics_list.append(thread)
	...
	prev = int(page_num)-1
	last = 100
	return render(request, 'posts/board.html', {'next' : next, 'prev' : prev, 'page_num': page_num, 'last' : last, 'curr_board': curr_board,'board_list': board_list, 'thread_form': thread_form, 'post_form': post_form, 'topics_list': topics_list, 'posts_list': posts_list})