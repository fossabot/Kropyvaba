...
// @version         16.6.17.0
// @namespace       https://www.freedollchan.org/scripts/*
...
var defaultCfg = {
	...
	'language':         0,      // script language [0=uk, 1=en]
	...
	'desktNotif':       1,      //    desktop notifications for new posts
	...
	'postBtnsCSS':      0,      // post buttons style [0=simple green, 1=gradient grey, 2=custom]
	'postBtnsBack':     '#8C8C8C',      // custom background color
	'noSpoilers':       0,      // expand text spoilers [0=off, 1=grey, 2=native]
	...
	'webmVolume':       70,    //    default volume for webm files (%)
	...
	'YTubeType':        1,      //    player type [0=flash, 1=HTML5]
	...
	'removeFName':      1,      //    remove file name
	...
	'noName':           1,      // hide form name field
	'noSubj':           0,      // hide form subject field
	'scriptStyle':      2,      // script style [0=gradient black, 1=gradient blue, 2=solid grey, 3=transparent blue]
...
Lng = {
		cfg: {
			'hideBySpell': ['Заклинання: ', 'Magic spells: '],
			'sortSpells': ['Сортувати заклинання й видаляти дублікати', 'Sort spells and delete duplicates'],
			'menuHiddBtn': ['Додаткове меню кнопок приховування', 'Additional menu of hide buttons'],
			'hideRefPsts': ['Приховувати відповіді на приховані дописи', 'Hide replies to hidden posts'],
			'delHiddPost': ['Видаляти приховані дописи', 'Delete hidden posts'],

			'ajaxUpdThr': ['Оновлення AJAX-у допису', 'AJAX thread update '],
			'updThrDelay': ['(сек)', '(sec)'],
			'updCount': ['Зворотній відлік секунд до оновлення', 'Show countdown to thread update'],
			'favIcoBlink': ['Блимати іконкою сторінки в разі появи нових дописів', 'Favicon blinking for new posts'],
			'desktNotif': ['Повідомляти про нові дописи на стільниці', 'Desktop notifications for new posts'],
			'noErrInTitle': ['Не показувати номер помилки в заголовку', 'Don\'t show error number in title'],
			'markNewPosts': ['Виділяти кольором нові дописи', 'Mark new posts with color'],
			'markMyPosts': ['Виділяти кольором мої дописи', 'Mark my posts with color'],
			'hideReplies': ['Показувати тільки OP-дописи в списку дописів*', 'Show only op-posts in threads list*'],
			'expandTrunc': ['Розгортати згорнуті дописи*', 'Auto expanding of truncated posts*'],
			'updThrBtns': ['Кнопки отримання нових дописів у списку ниток', 'Get-new-posts buttons in threads list'],
			'showHideBtn': ['Кнопки приховування ', 'Hide buttons '],
			'showRepBtn': ['Кнопки швидкої відповіді', 'Quick reply buttons'],
			'postBtnsCSS': {
				sel: [['Спрощені', 'Сірий градієнт', 'Користувацькі'], ['Simple green', 'Gradient grey', 'Custom filled']],
				txt: ['Кнопки дописів ', 'Post buttons ']
			},
			'noSpoilers': {
				sel: [['Вимк.', 'Сіре', 'Рідне'], ['Disable', 'Grey', 'Native']],
				txt: ['Розкривання тестових спойлерів', 'Text spoilers expanding']
			},
			'noPostNames': ['Приховувати імена в дописах', 'Hide names in posts'],
			'widePosts': ['Розтягувати дописи на ширину екрану', 'Stretch posts to screen width'],
			'hotKeys': ['Гарячі клавіші', 'Keyboard hotkeys'],
			'loadPages': ['Кількість сторінок, що завантажуються на F5', 'Number of pages that are loaded on F5 '],
			'correctTime': ['Корекція часу* ', 'Correct time* '],
			'timeOffset': ['(год) різниця ', '(h) difference '],
			'timePattern': ['Шаблон пошуку', 'Search pattern'],
			'timeRPattern': ['Шаблон заміни', 'Replace pattern'],

			'expandImgs': {
				sel: [['Вимк.', 'В дописі', 'По центру'], ['Disable', 'In post', 'By center']],
				txt: ['Розгортати зображення на клацання', 'Expand images on click']
			},
			'imgNavBtns': ['Додавати кнопки навігації по зображеннях', 'Add buttons for images navigation'],
			'resizeDPI': ['Не розтягувати зображення на дисплеях з високим DPI', 'Don\'t upscale images on retina displays'],
			'resizeImgs': ['Припасовувати великі зображення до екрана', 'Resize large images to fit screen'],
			'minImgSize': ['Мінімальний розмір зображень (пікс.)', 'Minimal image\'s size (px)'],
			'zoomFactor': ['Чутливість зуму зображень [1-100%]', 'Sensibility of the images zoom [1-100%]'],
			'webmControl': ['Показувати смугу керування для webm', 'Show control bar for webm files'],
			'webmTitles': ['Отримувати заголовки webm з метаданих', 'Get webm titles from metadata'],
			'webmVolume': ['Гучність webm [0-100%]', 'Default volume for webm files [0-100%]'],
			'preLoadImgs': ['Завантажувати зображення заздалегідь*', 'Pre-load images*'],
			'findImgFile': ['Розпізнавати вбудовані в зображення файли*', 'Detect built-in files in images*'],
			'openImgs': {
				sel: [['Вимк.', 'Всі', 'Тільки GIF', 'Крім GIF'], ['Disable', 'All types', 'Only GIF', 'Non-GIF']],
				txt: ['Заміняти зображення на оригінали*', 'Replace images with originals*']
			},
			'imgSrcBtns': ['Додавати кнопки для пошуку зображень', 'Add image search buttons'],
			'delImgNames': ['Приховувати імена зображень', 'Hide names of images'],
			'maskVisib': ['Видимість для маскованих зображень [0-100%]', 'Visibility for masked images [0-100%]'],

			'linksNavig': {
				sel: [['Вимк.', 'Без карти', 'З картою'], ['Disable', 'No map', 'With map']],
				txt: ['Навігація по >>посиланнях* ', 'Navigation by >>links* ']
			},
			'linksOver': ['Поява ', 'Appearance '],
			'linksOut': ['Зникнення (мс)', 'Disappearance (ms)'],
			'markViewed': ['Позначати переглянуті дописи', 'Mark viewed posts'],
			'strikeHidd': ['Закреслювати >>посилання на приховані дописи', 'Strike >>links to hidden posts'],
			'removeHidd': ['Видаляти з карти відповідей', 'Remove from replies map'],
			'noNavigHidd': ['Не показувати превʼю до прихованих дописів', 'Don\'t show previews for hidden posts'],
			'markMyLinks': ['Позначати посилання на мої дописи як (Ви)', 'Mark links to my posts with (You)'],
			'crossLinks': ['Перетворювати https:// на >>/b/посилання*', 'Replace https:// with >>/b/links*'],
			'insertNum': ['Вставляти >>посилання на клік по №допису*', 'Insert >>link on №postnumber click*'],
			'addOPLink': ['>>посилання при відповіді на OP-допис на дошці', 'Insert >>link for reply to op-posts on board'],
			'addImgs': ['Завантажувати зображення до посилань jpg, png, gif*', 'Load images to jpg, png, gif links*'],
			'addMP3': ['Програвач до посилань mp3* ', 'Player to mp3 links* '],
			'addVocaroo': ['до посилань Vocaroo*', 'to Vocaroo links*'],
			'addVimeo': ['Додавати програвач до посилань Vimeo*', 'Add player to Vimeo links*'],
			'addYouTube': {
				sel: [['Нічого', 'Програвач на посилання', 'Автопрограчач', 'Превʼю+програвач', 'Тільки превʼю'], ['Nothing', 'On click player', 'Auto player', 'Preview+player', 'Only preview']],
				txt: ['до посилань YouTube* ', 'to YouTube-links* ']
			},
			'YTubeType': {
				sel: [['Flash', 'HTML5'], ['Flash', 'HTML5']],
				txt: ['', '']
			},
			'YTubeTitles': ['Завантажувати назви до посилань YouTube*', 'Load titles into YouTube-links*'],
			'ytApiKey': ['Ключ YT API*', 'YT API Key*'],

			'ajaxReply': {
				sel: [['Вимк.', 'Iframe', 'HTML5'], ['Disable', 'Iframe', 'HTML5']],
				txt: ['Дописування через AJAX*', 'posting with AJAX*']
			},
			'postSameImg': ['Можливість надсилання однакових зображень', 'Ability to post same images'],
			'removeEXIF': ['Видаляти EXIF з JPEG ', 'Remove EXIF from JPEG '],
			'removeFName': ['Видаляти імена файлів', 'Clear file names'],
			'sendErrNotif': ['Сповіщати в заголовку про помилку надсилання', 'Inform in title about post send error'],
			'scrAfterRep': ['Переміщуватися в кінець нитки після надсилання', 'Scroll to the bottom after reply'],
			'addPostForm': {
				sel: [['Вгорі', 'Знизу', 'Прихована'], ['At top', 'At bottom', 'Hidden']],
				txt: ['Форма відповіді в нитці', 'Reply form in thread']
			},
			'spacedQuote': ['Вставляти пробіл під час цитування з "> "', 'Insert a space when quoting "> "'],
			'favOnReply': ['Додавати нитку в вибрані після відповідання', 'Add thread to favorites on reply'],
			'warnSubjTrip': ['Сповіщати в разі присутності трипкоду в полі «Тема»', 'Warn if "Subject" field contains trip-code'],
			'fileThumb': ['Область мініатюри зображень замість кнопки «Файл»', 'File thumbnail area instead of "File" button'],
			'addSageBtn': ['Кнопка Sage замість "E-mail" ', 'Sage button instead of "E-mail" '],
			'saveSage': ['Запамʼятовувати sage', 'Remember sage'],
			'capUpdTime': ['Інтервал оновлення капчі (сек)', 'Captcha update interval (sec)'],
			'captchaLang': {
				sel: [['Вимк.', 'Eng', 'Ukr'], ['Disable', 'Eng', 'Rus']],
				txt: ['Мова введення капчі', 'Language input in captcha']
			},
			'addTextBtns': {
				sel: [['Вимк.', 'Графіч.', 'Спрощ.', 'Стандарт.'], ['Disable', 'As images', 'As text', 'Standard']],
				txt: ['Кнопки розмітки тексту ', 'Text format buttons ']
			},
			'txtBtnsLoc': ['Знизу', 'At bottom'],
			'userPassw': ['Постійний пароль', 'Fixed password'],
			'userName': ['Постійне імʼя', 'Fixed name'],
			...
			'userCSS': ['Користувацький CSS', 'User CSS'],
			'panelCounter': ['Лічильник дописів/зображень на головній панелі', 'Counter of posts/images on main panel'],
			'rePageTitle': ['Назва нитки в заголовку вкладки*', 'Thread title in page tab*'],
			'animation': ['Анімація CSS3 в скрипті', 'CSS3 animation in script'],
			'closePopups': ['Автоматично закривати сповіщення', 'Close popups automatically'],
			'inftyScroll': ['Нескінченна прокрутка сторінок', 'Infinity scroll for pages'],
			'scrollToTop': ['Завжди переходити нагору в списку дописів', 'Always scroll to top in threads list'],
			'updScript': ['Автоматично перевіряти оновлення скрипта', 'Check for script update automatically'],
			'scrUpdIntrv': {
				sel: [['Щодня', 'Кожні 2 дні', 'Щотижня', 'Кожні 2 тижні', 'Щомісяця'], ['Every day', 'Every 2 days', 'Every week', 'Every 2 week', 'Every month']],
				txt: ['', '']
			},
			'excludeList': ['Не запускати скрипт на:', 'Prevent script launch on:'],
			'turnOff': ['Вмикати скрипт тільки на цьому сайті', 'Enable script only on this site'],
...
txtBtn: [['Жирний', 'Bold'], ['Курсив', 'Italic'], ['Підкреслений', 'Underlined'], ['Закреслений', 'Strike'], ['Спойлер', 'Spoiler'], ['Код', 'Code'], ['Верхній індекс', 'Superscript'], ['Нижній індекс', 'Subscript'], ['Цитувати виділене', 'Quote selected']],
		cfgTab: {
			'filters': ['Фільтри', 'Filters'],
			'posts': ['Дописи', 'Posts'],
			'images': ['Зображення', 'Images'],
			'links': ['Посилання', 'Links'],
			'form': ['Форма', 'Form'],
			'common': ['Спільне', 'Common'],
			'info': ['Інфо', 'Info']
		},

		panelBtn: {
			'attach': ['Закріпити/відкріпити панель', 'Attach/Detach panel'],
			'cfg': ['Налаштування', 'Settings'],
			'hid': ['Приховане', 'Hidden'],
			'fav': ['Вибране', 'Favorites'],
			'vid': ['Посилання на відео', 'Video links'],
			'refresh': ['Оновити', 'Refresh'],
			'goback': ['Назад на дошку', 'Return to board'],
			'gonext': ['На сторінку %s', 'Go to page %s'],
			'goup': ['До початку сторінки', 'To top of page'],
			'godown': ['До кінця сторінки', 'To bottom of page'],
			'expimg': ['Розгорнути всі зображення', 'Expand all images'],
			'preimg': ['Попереднє завантаження зображень ([Ctrl+Click] тільки для нових дописів)', 'Preload images ([Ctrl+Click] for new posts only)'],
			'maskimg': ['Маскувати зображення', 'Mask images'],
			'upd-on': ['Вимкнути автооновлення нитки', 'Disable thread autoupdate'],
			'upd-off': ['Увімкнути автоооновлення нитки', 'Enable thread autoupdate'],
			'audio-off': ['Звукове сповіщення про нові дописи', 'Sound notification about new posts'],
			'catalog': ['Перейти до каталога', 'Go to catalog'],
			'pcount': ['Дописів у нитці', 'Posts in thread'],
			'imglen': ['Зображень у нитці', 'Images in thread'],
			'posters': ['Дописувачів у нитці', 'Posters in thread'],
			'savethr': ['Зберегти на диск', 'Save to disk'],
			'enable': ['Увімкнути/вимкнути скрипт', 'Turn on/off the script']
		},

		selHiderMenu: {
			'sel': ['Приховувати виділене', 'Hide selected text'],
			'name': ['Приховувати імʼя', 'Hide name'],
			'trip': ['Приховувати трипкод', 'Hide with trip-code'],
			'img': ['Приховувати зображення', 'Hide with image'],
			'ihash': ['Приховувати схожі зображення', 'Hide similar images'],
			'noimg': ['Приховувати без зображень', 'Hide without images'],
			'notext': ['Приховувати без тексту', 'Hide without text'],
			'text': ['Приховати схожий текст', 'Hide similar text'],
			'refs': ['Приховати з відповідями', 'Hide with answers']
		},
		selExpandThr: [['+10 дописів', 'Останні 30', 'Останні 50', 'Останні 100', 'Вся нитка'], ['+10 posts', 'Last 30 posts', 'Last 50 posts', 'Last 100 posts', 'All thread']],
		selAjaxPages: [['1 сторінка', '2 сторінки', '3 сторінки', '4 сторінки', '5 сторінок'], ['1 page', '2 pages', '3 pages', '4 pages', '5 pages']],
		selSaveThr: [['Завантажити всю нитку', 'Завантажити зображення'], ['Download thread', 'Download images']],
		selAudioNotif: [['Кожні 30 сек.', 'Щохвилини', 'Кожні 2 хв.', 'Кожні 5 хв.'], ['Every 30 sec.', 'Every minute', 'Every 2 min.', 'Every 5 min.']],

		hotKeyEdit: [['%l%i24 – попередня сторінка/зображення%/l', '%l%i217 – наступна сторінка/зображення%/l', '%l%i21 – нитка (на дошці)/допис (у нитці) нижче%/l', '%l%i20 – нитка (на дошці)/допис (у нитці) вище%/l', '%l%i31 – допис (на дошці) нижче%/l', '%l%i30 – допис (на дошці) вище%/l', '%l%i23 – приховати допис/нитку%/l', '%l%i32 – перейти до нитки%/l', '%l%i33 – розгорнути нитку%/l', '%l%i211 – розгорнути зображення в дописі%/l', '%l%i22 – швидка відповідь%/l', '%l%i25t – надіслати допис%/l', '%l%i210 – відкрити/закрити «Налаштування»%/l', '%l%i26 – відкрити/закрити «Вибране»%/l', '%l%i27 – відкрити/закрити «Приховане»%/l', '%l%i218 – відкрити/закрити «Відео»%/l', '%l%i28 – відкрити/закрити панель%/l', '%l%i29 – увімк./вимк. маскування зображень%/l', '%l%i40 – оновити нитку (в нитці)%/l', '%l%i212t – жирний%/l', '%l%i213t – курсив%/l', '%l%i214t – закреслений%/l', '%l%i215t – спойлер%/l', '%l%i216t – код%/l'], ['%l%i24 – previous page/image%/l', '%l%i217 – next page/image%/l', '%l%i21 – thread (on board)/post (in thread) below%/l', '%l%i20 – thread (on board)/post (in thread) above%/l', '%l%i31 – on board post below%/l', '%l%i30 – on board post above%/l', '%l%i23 – hide post/thread%/l', '%l%i32 – go to thread%/l', '%l%i33 – expand thread%/l', '%l%i211 – expand post\'s images%/l', '%l%i22 – quick reply%/l', '%l%i25t – send post%/l', '%l%i210 – open/close "Settings"%/l', '%l%i26 – open/close "Favorites"%/l', '%l%i27 – open/close "Hidden"%/l', '%l%i218 – open/close "Videos"%/l', '%l%i28 – open/close main panel%/l', '%l%i29 – turn on/off masking images%/l', '%l%i40 – update thread%/l', '%l%i212t – bold%/l', '%l%i213t – italic%/l', '%l%i214t – strike%/l', '%l%i215t – spoiler%/l', '%l%i216t – code%/l']],

		month: [['січ', 'лют', 'бер', 'кві', 'тра', 'чер', 'лип', 'сер', 'вер', 'жов', 'лис', 'гру'], ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']],
		fullMonth: [['січня', 'лютого', 'березня', 'квітня', 'травня', 'червня', 'липня', 'серпня', 'вересня', 'жовтня', 'листопада', 'грудня'], ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']],
		week: [['Нед', 'Пон', 'Вів', 'Сер', 'Чет', 'Птн', 'Сбт'], ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']],

		editor: {
			cfg: ['Редагування налаштувань:', 'Edit settings:'],
			hidden: ['Редагування прихованих ниток:', 'Edit hidden threads:'],
			favor: ['Редагування вибраного:', 'Edit favorites:'],
			css: ['Редагування CSS', 'Edit CSS']
		},

		newPost: [[' новий допис', ' нових дописи', ' нових дописів', '. Останній:'], [' new post', ' new posts', ' new posts', '. Latest: ']],

		name: ['Імʼя', 'Name'],
		subj: ['Тема', 'Subject'],
		mail: ['Ел. пошта', 'E-mail'],
		cap: ['Капча', 'Captcha'],
		video: ['Відео', 'Video'],
		add: ['Додати', 'Add'],
		...
		refresh: ['Оновити', 'Refresh'],
		load: ['Завантажити', 'Load'],
		save: ['Зберегти', 'Save'],
		edit: ['Правка', 'Edit'],
		file: ['Файл', 'File'],
		global: ['Глобальні', 'Global'],
		reset: ['Скинути', 'Reset'],
		remove: ['Видалення', 'Removing'],
		info: ['Інфо', 'Info'],
		undo: ['Скасувати', 'Undo'],
		change: ['Змінити', 'Change'],
		reply: ['Відповідь', 'Reply'],
		loading: ['Завантаження...', 'Loading...'],
		checking: ['Перевірка...', 'Checking...'],
		deleting: ['Видалення...', 'Deleting...'],
		updating: ['Оновлення...', 'Updating...'],
		error: ['Помилка', 'Error'],
		noConnect: ['Помилка зʼєднання', 'Connection failed'],
		internalError: ['Помилка скрипта:\n', 'Script error:\n'],
		thrNotFound: ['Нитка недоступна (№', 'Thread is unavailable (№'],
		thrClosed: ['Нитку закрито', 'Thread is closed'],
		succDeleted: ['Успішно видалено!', 'Succesfully deleted!'],
		errDelete: ['Не можу видалити:\n', 'Can\'t delete:\n'],
		cTimeError: ['Неправильні налаштування часу', 'Invalid time settings'],
		noGlobalCfg: ['Глобальні налаштування не знайдено', 'Global config not found'],
		postNotFound: ['Допис не знайдено', 'Post not found'],
		dontShow: ['Приховати: ', 'Hide: '],
		checkNow: ['Перевірити зараз', 'Check now'],
		updAvail: ['Доступне оновлення!', 'Update available!'],
		haveLatest: ['У вас остання версія!', 'You have latest version!'],
		thrViewed: ['Ниток відвідано', 'Threads visited'],
		thrCreated: ['Ниток створено', 'Threads created'],
		thrHidden: ['Ниток приховано', 'Threads hidden'],
		postsSent: ['Дописів надіслано', 'Posts sent'],
		total: ['Всього', 'Total'],
		debug: ['Зневадження', 'Debug'],
		infoDebug: ['Інформація для зневадження', 'Information for debugging'],
		impexpCfg: ['Імпорт/експорт налаштувань', 'Config import/export'],
		fileToCfg: ['Завантажити налаштування з файла', 'Load config from a file'],
		cfgToFile: ['Зберегти файл</a> з налаштуваннями', 'Get config file</a>'],
		globalCfg: ['Глобальні налаштування', 'Global config'],
		loadGlobal: [' і застосувати до цього домена', ' and apply to this domain'],
		saveGlobal: [' поточні налаштування як глобальні', ' current config as global'],
		descrGlobal: ['Глобальні налаштування будуть за промовчанням застосовуватися<br>під час першого відвідання інших доменів', 'Global config will apply by default<br>at the first visit of other domains'],
		editInTxt: ['Правка в текстовому форматі', 'Edit in text format'],
		resetCfg: ['Скинути в налаштування за промовчанням', 'Reset config to defaults'],
		resetData: ['Очистити дані', 'Reset selected data'],
		allDomains: ['Для всіх доменів:', 'For all domains'],
		clrSelected: ['Видалити виділені записи', 'Remove selected notes'],
		saveChanges: ['Зберегти внесені зміни', 'Save your changes'],
		infoCount: ['Оновити лічильники дописів', 'Refresh posts counters'],
		infoPage: ['Перевірити актуальність ниток (до 10 сторінки)', 'Check for threads actuality (up to 10 page)'],
		clrDeleted: ['Очистити недоступні (404) нитки', 'Clear inaccessible (404) threads'],
		oldPosts: ['Дописів під час останнього відвідування', 'Posts at the last visit'],
		newPosts: ['Кількість нових дописів', 'Number of new posts'],
		thrPage: ['Нитка на @сторінці', 'Thread on @page'],
		hiddenPosts: ['Приховані дописи', 'Hidden posts'],
		onPage: [' на сторінці', ' on the page'],
		hiddenThrds: ['Приховані нитки', 'Hidden threads'],
		myPosts: ['Мої дописи', 'My posts'],
		noHidPosts: ['На цій сторінці немає прихованих дописів...', 'No hidden posts on this page...'],
		noHidThrds: ['Немає прихованих дописів...', 'No hidden threads...'],
		expandAll: ['Розгорнути все', 'Expand all'],
		invalidData: ['Некоректний формат даних', 'Incorrect data format'],
		noFavThrds: ['Немає вибраних ниток...', 'Favorites is empty...'],
		noVideoLinks: ['Немає посилань на відео...', 'No video links...'],
		hideLnkList: ['Приховати/показати список посилань', 'Hide/Unhide list of links'],
		prevVideo: ['Попереднє відео', 'Previous video'],
		nextVideo: ['Наступне відео', 'Next video'],
		toPanel: ['Закріпити на панель', 'Attach to panel'],
		underPost: ['Помістити форму під допис', 'Move under post'],
		makeDrag: ['Зробити перетягуваним вікном', 'Make draggable window'],
		closeWindow: ['Закрити вікно', 'Close window'],
		closeReply: ['Закрити форму', 'Close form'],
		replies: ['Відповіді:', 'Replies:'],
		postsOmitted: ['Пропущено відповідей: ', 'Posts omitted: '],
		showPosts: ['Показати дописи', 'Show posts'],
		hidePosts: ['Приховати дописи', 'Hide posts'],
		collapseThrd: ['Згорнути нитку', 'Collapse thread'],
		deleted: ['видалено', 'deleted'],
		getNewPosts: ['Отримати нові дописи', 'Get new posts'],
		page: ['Сторінка', 'Page'],
		hiddenThrd: ['Прихована нитка:', 'Hidden thread:'],
		makeThrd: ['Створити нитку', 'Create thread'],
		makeReply: ['Відповісти', 'Make reply'],
		noSage: ['Без sage', 'No sage'],
		hideForm: ['Приховати форму', 'Hide form'],
		search: ['Шукати в ', 'Search in '],
		wait: ['Чекайте', 'Wait'],
		noFile: ['Немає файла', 'No file'],
		clickToAdd: ['Виберіть або перетягніть файл', 'Select or drag and drop file'],
		removeFile: ['Видалити файл', 'Remove file'],
		spoilFile: ['Спойлер', 'Spoiler'],
		helpAddFile: ['Вбудувати .ogg, .rar, .zip або .7z в зображення', 'Pack .ogg, .rar, .zip or .7z into image'],
		downloadFile: ['Завантажити файл, що міститься в зображення', 'Download existing file from image'],
		fileCorrupt: ['Файл пошкоджено: ', 'File is corrupted: '],
		subjHasTrip: ['Поле «Тема» містить трипкод', '"Subject" field contains a tripcode'],
		loadImage: ['Завантажуються зображення: ', 'Loading images: '],
		loadFile: ['Завантажуються файли: ', 'Loading files: '],
		cantLoad: ['Не можу завантажити ', 'Can\'t load '],
		willSavePview: ['Буде збережено мініатюру', 'Thumbnail will be saved'],
		loadErrors: ['Під час завантаження сталися помилки:', 'An error occurred during the loading:'],
		errCorruptData: ['Помилка: сервер надіслав пошкоджені дані', 'Error: server sent corrupted data'],
		expImgInline: ['[Click] розгорнути в дописі, [Ctrl+Click] в центрі', '[Click] expand in post, [Ctrl+Click] by center'],
		expImgFull: ['[Click] розгорнути в центрі, [Ctrl+Click] в дописі', '[Click] expand by center, [Ctrl+Click] in post'],
		nextImg: ['Наступне зображення', 'Next image'],
		prevImg: ['Попереднє зображення', 'Previous image'],
		togglePost: ['Приховати/показати допис', 'Hide/Unhide post'],
		replyToPost: ['Відповісти на допис', 'Reply to post'],
		expandThrd: ['Розгорнути нитку', 'Expand thread'],
		addFav: ['Додати нитку в вибране', 'Add thread to Favorites'],
		delFav: ['Прибрати нитку з вибраного', 'Remove thread from Favorites'],
		attachPview: ['Закріпити превʼю', 'Attach preview'],
		expandVideo: ['Розгорнути/згорнути відео', 'Expand/Collapse video'],
		duration: ['Тривалість: ', 'Duration: '],
		published: ['опубліковано: ', 'published: '],
		author: ['Автор: ', 'Author: '],
		views: ['переглядів: ', 'views: '],

		seSyntaxErr: ['синтаксична помилка в аргументі закляття: %s', 'syntax error in argument of spell: %s'],
		seUnknown: ['невідоме закляття: %s', 'unknown spell: %s'],
		seMissOp: ['пропущено оператор', 'missing operator'],
		seMissArg: ['пропущено аргумент закляття: %s', 'missing argument of spell: %s'],
		seMissSpell: ['пропущено закляття', 'missing spell'],
		seErrRegex: ['синтаксична помилка в регулярному виразі: %s', 'syntax error in regular expression: %s'],
		seUnexpChar: ['неочікуваний символ символ: %s', 'unexpected character: %s'],
		seMissClBkt: ['пропущено закривну дужку', 'missing ) in parenthetical'],
		seRepsInParens: ['закляття $s не може бути в дужках', 'spell %s shouldn\'t be in parens'],
		seOpInReps: ['неприпустимо використовувати оператор %s з закляттями #rep и #outrep', 'don\'t use operator %s with spells #rep & #outrep'],
		seRow: [' (рядок ', ' (row '],
		seCol: [', стовпчик ', ', column '],
		sendingPost: ['Надсилання допису...', 'Sending post...'],
		sizeByte: [' Байт', ' Byte'],
		sizeKByte: [' КБ', ' KB'],
		sizeMByte: [' МБ', ' MB'],
		sizeGByte: [' ГБ', ' GB'],
...
function* readCfg() {
	...
	defaultCfg.language = +!String(navigator.language).toLowerCase().startsWith('uk');
		...
		for(var b in data[h]) {
			...
			for(var tNum in d) {
				...
				if(!t.url.startsWith('http')) {
					t.url = (h === aib.host ? aib.prot + '//' : 'https://') + h + t.url;
				}
				innerHtml += `
...
function getCfgFilters() {
	return $New('div', {'class': 'de-cfg-unvis', 'id': 'de-cfg-filters'}, [
		$New('div', {'id': 'de-spell-panel'}, [
			...
			$add('<a href="' + gitWiki + 'Spells-' + (lang ? 'en' : 'uk') +
...
function getCfgPosts() {
	return $New('div', {'class': 'de-cfg-unvis', 'id': 'de-cfg-posts'}, [
		...
		$New('div', null, [
			...
			$add('<a href="' + gitWiki + 'Settings-time-' + (lang ? 'en' : 'uk') +
...
function getCfgLinks() {
	return $New('div', {'class': 'de-cfg-unvis', 'id': 'de-cfg-links'}, [
		...
		$if(aib.prot === 'https:', lBox('addVocaroo', false, null)),
...
function getCfgInfo() {
	...
	return $New('div', {'class': 'de-cfg-unvis', 'id': 'de-cfg-info'}, [$add(`
		<div style="padding-bottom: 10px;">
			...
			&nbsp;|&nbsp;
			<a href="https://www.freedollchan.org/scripts/" target="_blank">Freedollchan</a>
			&nbsp;|&nbsp;
		...
		return txt.replace(new RegExp(this.regex, 'g'), (str, ...args) => {
			...
			for(var i = 0; i < 7; ++i) {
				...
				case 'm':
					...
					case 'січ': case 'jan': month = 0; break;
					case 'лют': case 'feb': month = 1; break;
					case 'бер': case 'mar': month = 2; break;
					case 'квіт': case 'apr': month = 3; break;
					case 'трав': case 'мая': case 'may': month = 4; break;
					case 'черв': case 'jun': month = 5; break;
					case 'лип': case 'jul': month = 6; break;
					case 'сер': case 'aug': month = 7; break;
					case 'вер': case 'sep': month = 8; break;
					case 'жов': case 'oct': month = 9; break;
					case 'лист': case 'nov': month = 10; break;
					case 'груд': case 'dec': month = 11; break;
		...
		for(var i = 0, len = els.length; i < len; ++i) {
			...
			if(!el || el.className !== 'de-vocaroo') {
				link.insertAdjacentHTML('beforebegin', '<div class="de-vocaroo"><embed' +
					...
					' src="https://vocaroo.com/player.swf?playMediaID=' + src + '"></div>');
			}
		}
	}
}
...
function PostForm(form, oeForm = null, ignoreForm = false) {
	...
	this.subm.addEventListener('click', e => {
		...
		if(this.video && (val = this.video.value) && (val = val.match(Videos.ytReg))) {
			this.video.value = 'https://www.youtube.com/watch?v=' + val[1];
		}
		if(this.isQuick) {
			$hide(this.pForm);
			$hide(this.qArea);
...
PostForm.prototype = {
	...
	addTextPanel() {
		...
		`<span id="de-btn-quote" de-title="${ Lng.txtBtn[i][lang] }" de-tag="q">${
		...
		if(this.isVideo) { // FIXME: handle null size videos
			...
			if(nav.canPlayWebm) {
				...
				obj = $add('<object style="width: inherit; height: inherit" data="' + src +
					...
					'<param name="pluginspage" value="https://www.videolan.org/vlc/"/>' +
...
class AbstractPost {
	...
	_getMenuImgSrc(el) {
		...
		return '<a class="de-menu-item de-src-google" href="https://www.google.com/searchbyimage?image_url=' + p + 'Google</a>' +
			'<a class="de-menu-item de-src-yandex" href="https://yandex.ru/images/search?rpt=imageview&img_url=' + p + 'Yandex</a>' +
			'<a class="de-menu-item de-src-tineye" href="https://tineye.com/search/?url=' + p + 'TinEye</a>' +
			'<a class="de-menu-item de-src-saucenao" href="https://saucenao.com/search.php?url=' + p + 'SauceNAO</a>' +
			'<a class="de-menu-item de-src-iqdb" href="https://iqdb.org/?url=' + p + 'IQDB</a>' +
			'<a class="de-menu-item de-src-whatanime" href="https://whatanime.ga/?auto&url=' + (aib.constructor.name === 'Iichan' ? 'https://reho.st/' + p : p) + 'WhatAnime</a>';
	}
	_showMenu(el, html) {
		if(this._menu) {
			this._menu.remove();
	...
	getPostHTML(i) {
		...
		for(let file of data.files) {
			...
			let fileInfo = `<div class="fileinfo${ multiFile ? ' limited' : '' }">Файл:
				...
				<a class="search_google icon" href="https://www.google.com/searchbyimage?image_url=https://dobrochan.ru/${ file.src }"><img title="edit" alt="edit" src="/images/blank.png"></a>
				<a class="search_iqdb icon" href="https://iqdb.org/?url=https://dobrochan.ru/${ file.src }"><img title="edit" alt="edit" src="/images/blank.png"></a>
			</div>`;
...
class MakabaPostsBuilder {
	...
	* bannedPostsData() {
		for(let post of this._posts) {
			switch(post.banned) {
			case 1:
				yield [1, post.num, $add('<span class="pomyanem">(Автора цього допису забанено. Пом’янемо.)</span>')];
				break;
			case 2:
				yield [2, post.num, $add('<span class="pomyanem">(Автора цього допису попереджено.)</span>')];
				break;
			}
		}
	}
	...
	_getPostMsg(data) {
		...
		return `<blockquote id="m${ data.num }" class="post-message">
			${ comment }
			${ _switch(data.banned, {
			   1:           '<br/><span class="pomyanem">(Автора цього допису забанено. Пом’янемо.)</span>',
			   2:           '<br/><span class="pomyanem">(Автора цього допису попереджено.)</span>',
			   '@@default': ''
			}) }
		</blockquote>`;
	}
...
function getImageBoard(checkDomains, checkEngines) {
	...
	class Makaba extends BaseBoard {
		...
		getBanId(postEl) {
			...
			return el.textContent.includes('попереджений') ? 2 : 1;
		}
		getImgParent(node) {
		...
		updateCaptcha(cap, isErr) {
			...
			return this._capUpdPromise = $ajax(
				...
				if(data.includes('VIPFAIL')) {
					el.innerHTML = 'Ваш пас-код не дійсний, будь ласка, переввійдіть. <a href="#" id="renew-pass-btn">Оновити</a>';
				} else if(data.includes('VIP')) {
					el.innerHTML = 'Вам не потрібно вводити капчу, у вас введено пас-код.';
				...
				`<span id="de-_2chruNet-capchecker" class="shortened" style="margin: 0px .5em;">
					...
					$ajax('/' + this.b + '/api/validate-captcha', { method: 'POST' }).then(xhr => {
						if(JSON.parse(xhr.responseText).status === 'ok') {
							target.innerHTML = 'можна дописувати';
						} else {
							target.innerHTML = 'неправильна капча';
							setTimeout(() => target.innerHTML = 'перевірити капчу', 1e3);
						}
					}, emptyFn);
				};
			}, e => {
		...
		fixHTML(data, isForm) {
			...
			try {
				...
				for(let i = 0; i < tLen; ++i) {
					...
					const link = thumb.getAttribute('onclick').match(/https:\/[^']+/)[0];
	...
	class DvaChNet extends BaseBoard {
		...
		init() {
			...
			if(el) {
				...
				$replace(el, '<input type="submit" id="submit" name="submit" value="Відповідь"/>');
			}
			return false;
		}
		updateCaptcha() {
